...
print(f"Total prod_id: {df['prod_id'].nunique()}")
...
while True:
    ...
    if data.empty:
        print("prod_id not found")
        continue

    fig, axes = plt.subplots(1, 2, figsize=(16, 6))
    cat = data["category"].iloc[0] if "category" in data.columns else ""
    fig.suptitle(f"prod_id: {value}  category: {cat}")
